...
timeofday = input(:,3); %Only shows the time hourly each day
time = time'; %time in hours for dataset
deltaTime = 1; % time increment IN HOURS

%BESS variables
energyCapBESS = 120; % MWh maximum capacity
initialEnergyBESS = 40; % MWh initial capacity
chargePowerCap = 40; %MW BESS charge Power Cap
dischargePowerCap = 40; %MW BESS discharge Power Cap
...
startcharge = 1; %time in which batteries will start scheduled charge
endcharge = 7; %time in which batteries will end scheduled charge
startdischarge = 16; %time when batteries will start scheduled discharge
enddischarge = 20; %time when batteries will end scheduled discharge
chargeViaSolarThreshold = 70; %percent of NP Capacity production at which solar production would otherwise be curtailed

arraySize = 30; % capacity of solar array in MW

%substation overload variables
npCapacity = 130; %MW - nameplate rating of the substation transformer
...
% [netCostsCO2, annualCostsCO2, netCostsUSD, annualCostsUSD] = calcCosts(energyLoad,energySolar,percLoadGrowth,percSolarDeg,sizeSolarMW,sizeBESSMWh,substRatingMW,substUpgradeMW,durationOverloads,isDamaging);
