function plotOverloads2(figNum,load,npCapacity,overloads,timeOverload,isDamaging, isYearView, titleText)
...
%mark points where damaging overloads occur
for i = 1:length(timeOverload)
    if isDamaging(i) == 1
        timeStamp = timeOverload(i);
        plot(timeStamp,load(timeStamp),'rs');
    end
end
